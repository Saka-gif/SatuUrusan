...
import { Eye, EyeOff, ArrowRight, Lock, Mail, User, Phone, AlertCircle, LoaderCircle } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const name = String(form.get("name") || "Teman Satu").trim();
    const email = String(form.get("email") || "").trim().toLowerCase();
    setIsLoading(true);
    window.localStorage.setItem("satuurusan_account", JSON.stringify({
      name,
      email,
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5 pt-1 text-left">
        ...
        <div className="space-y-1">
          ...
          {confirmPassword && password !== confirmPassword && (
            <p className="text-[11px] font-semibold text-rose-600" role="status">Konfirmasi kata sandi belum cocok.</p>
          )}
        </div>
